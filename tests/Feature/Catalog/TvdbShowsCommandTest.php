...
use Tests\Support\TvdbShowsCommandTestHost;
...
|
| This file pins it directly on Tests\Support\TvdbShowsCommandTestHost, a
| throwaway host that extends the base and implements its one abstract hook
| (ids()) as "return what I was constructed with", so a test drives an exact id
| set through the real pipeline. The host carries its own #[Signature] (Symfony
| refuses to construct a Command without one) under a name no real command uses,
| so it can't collide in the registry.
|
