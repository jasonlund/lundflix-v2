...
files (`.claude/skills/tdd/SKILL.md:106`), so it cannot dedupe against a sibling it
...
| Severity | When |
|---|---|
| SHOULD_FIX | A whole method, class-level helper, or query shape duplicated across files; a load-bearing rationale comment duplicated verbatim (it will drift). |
| CONSIDER | A docblock, constant block, or Arrange block repeated; a third repetition that now justifies extraction. |

**Never BLOCKING.** Duplication does not break behavior, and BLOCKING is reserved
by the contract for a fixed list this is not on.
